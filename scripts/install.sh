...
else echo "Missing checksum tool (shasum or sha256sum)" >&2; exit 1; fi
...
(
  cd "$tmp_dir"
  expected_line="$(grep "  ${asset}$" checksums.txt || true)"
  [ -n "$expected_line" ] || { echo "Checksum entry not found for ${asset}" >&2; exit 1; }
  actual_line="$($sha_cmd "$asset")"
  [ "${expected_line%% *}" = "${actual_line%% *}" ] || { echo "Checksum verification failed for ${asset}" >&2; exit 1; }
)
